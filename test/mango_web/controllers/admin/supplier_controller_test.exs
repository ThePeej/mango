defmodule MangoWeb.Admin.SupplierControllerTest do
  ...
  describe "index" do
    test "lists all suppliers", %{conn: conn} do
      conn = get(conn, Routes.admin_supplier_path(conn, :index))
  ...
  describe "new supplier" do
    test "renders form", %{conn: conn} do
      conn = get(conn, Routes.admin_supplier_path(conn, :new))
  ...
  describe "create supplier" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, Routes.admin_supplier_path(conn, :create), supplier: @create_attrs)

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == Routes.admin_supplier_path(conn, :show, id)

      conn = get(conn, Routes.admin_supplier_path(conn, :show, id))
      assert html_response(conn, 200) =~ "Show Supplier"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.admin_supplier_path(conn, :create), supplier: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Supplier"
    end
  end
  ...
  describe "edit supplier" do
    ...
    test "renders form for editing chosen supplier", %{conn: conn, supplier: supplier} do
      conn = get(conn, Routes.admin_supplier_path(conn, :edit, supplier))
  ...
  describe "update supplier" do
    ...
    test "redirects when data is valid", %{conn: conn, supplier: supplier} do
      conn = put(conn, Routes.admin_supplier_path(conn, :update, supplier), supplier: @update_attrs)
      assert redirected_to(conn) == Routes.admin_supplier_path(conn, :show, supplier)

      conn = get(conn, Routes.admin_supplier_path(conn, :show, supplier))
    ...
    test "renders errors when data is invalid", %{conn: conn, supplier: supplier} do
      conn = put(conn, Routes.admin_supplier_path(conn, :update, supplier), supplier: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Supplier"
    end
  end
  ...
  describe "delete supplier" do
    ...
    test "deletes chosen supplier", %{conn: conn, supplier: supplier} do
      conn = delete(conn, Routes.admin_supplier_path(conn, :delete, supplier))
      assert redirected_to(conn) == Routes.admin_supplier_path(conn, :index)
      assert_error_sent 404, fn ->
        get(conn, Routes.admin_supplier_path(conn, :show, supplier))
      end
    end
  end
